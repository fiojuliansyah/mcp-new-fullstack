    ...
    <div class="space-y-7 flex flex-col">
        ...
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
            <div class="flex items-center gap-3">
                <a href="{{ route('admin.replays.index') }}" class="bg-white rounded-full w-8 h-8 flex items-center justify-center cursor-pointer">
                    <img src="/admin/assets/icons/arrow-left.svg" alt="Icon" class="size-4">
                </a>
                <h6 class="text-gray-100 font-[600]">View Replay Video</h6>
            </div>
        </div>
    </div>
    ...
    <div class="space-y-10 divide-y divide-gray-200">
        <div class="space-y-5">
                ...
                <table
                    ...
                    <thead class="bg-gray-800 text-gray-200">
                        <tr>
                            ...
                            <th class="p-4 border border-[#424242]">Total Videos</th>
                            <th class="p-4 border border-[#424242]">Action</th>
                        </tr>
                    </thead>
                    <tbody class="bg-gray-900 divide-y divide-gray-800 text-gray-50">
                        @forelse ($replays as $index => $replay)
                            <tr class="odd:bg-[#141414] even:bg-[#171717]">
                                <td class="p-4 border border-gray-700">{{ $replays->firstItem() + $index }}</td>
                                <td class="p-4 border border-gray-700">{{ $replay->schedule->topic ?? '-' }}</td>
                                <td class="p-4 border border-gray-700">{{ $replay->start_date->format('d M Y') ?? '-' }}</td>
                                <td class="p-4 border border-gray-700">{{ $replay->end_date ? $replay->end_date->format('d M Y') : '-' }}</td>
                                <td class="p-4 border border-gray-700">{{ $replay->created_at->format('d M Y') ?? '-' }}</td>
                                <td class="p-4 border border-gray-700">{{ $replay->replayVideos->count() ?? '0' }}</td>
                                <td class="p-4 border border-gray-700">
                                    <div class="flex items-center justify-center gap-3">
                                        <a href="{{ route('admin.replays.edit', $replay->id) }}" class="flex flex-col items-center">
                                        ...
                                        <button type="button" 
                                                data-id="{{ $replay->id }}" 
                                                data-topic="{{ $replay->schedule->topic_name ?? 'Replay' }}"
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="p-4 text-center text-gray-400 italic">No replays found for this schedule.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            <div class="flex items-center justify-center lg:justify-end gap-5">
                ...
                @else
                    <a href="{{ $replays->previousPageUrl() }}" class="bg-white rounded-full w-10 h-10 flex items-center justify-center">
                        <img src="/frontend/assets/icons/angle-left.svg" alt="Prev" class="size-3">
                    </a>
                @endif
                ...
                @if($replays->hasMorePages())
                    <a href="{{ $replays->nextPageUrl() }}" class="bg-white rounded-full w-10 h-10 flex items-center justify-center">
...
<div id="deleteModal" class="fixed inset-0 bg-black bg-opacity-75 hidden items-center justify-center z-50 transition-opacity duration-300">
    <div class="bg-[#141414] rounded-xl p-6 w-full max-w-sm mx-4 space-y-4 border border-gray-700">
        ...
        <p class="text-gray-300">Are you sure you want to delete the replay for topic: <span id="modalTopicName" class="font-bold text-white"></span>? This action cannot be undone, and associated videos will be permanently removed.</p>
...
<script>
    ...
    const baseUrl = '{{ route('admin.replays.destroy', 0) }}';
    ...
    document.querySelectorAll('.delete-button').forEach(button => {
        button.addEventListener('click', function() {
            const replayId = this.getAttribute('data-id');
            const topicName = this.getAttribute('data-topic');
            openModal(replayId, topicName);
        });
    });
