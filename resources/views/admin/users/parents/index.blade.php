    ...
    <div class="space-y-7 flex flex-col">
        ...
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
            <div>
                ...
                <p class="text-gray-910">{{ $count }}  Parents</p>
            </div>
                ...
                <table
                    ...
                    <thead class="bg-gray-800 text-gray-200">
                        <tr>
                            ...
                            <th class="p-4 border border-[#424242]">Total Childen</th>
                    ...
                    <tbody class="bg-gray-900 divide-y divide-gray-800 text-gray-50">
                        @foreach ($users as $index => $user)
                            @php
                                $totalSubjects = 0;                               
                                $subjectIds = $user->children->flatMap(function ($child) {
                                    return $child->subscriptions->flatMap(function ($subscription) {
                                        return $subscription->classrooms->map(function ($classroom) {
                                            return $classroom->subject->id ?? null;
                                        });
                                    });
                                })
                                ->filter()
                            ...
                            <tr class="odd:bg-[#141414] even:bg-[#171717]">
                                ...
                                <td class="p-4 border border-gray-700">{{ $user->children->count() ?? 0 }}</td>
                                
                                <td class="p-4 border border-gray-700">
                                    <span class="font-semibold text-blue-300">{{ $totalSubjects }}</span>
                                </td>
                                ...
                                <td class="p-4 border border-gray-700">
                                    <div class="flex items-center justify-center gap-3">
                                        <a href="{{ route('admin.users.parents.show', $user->id) }}" target="_blank" class="flex flex-col items-center">
                                        ...
                                        <a href="{{ route('admin.users.parents.edit', $user->id) }}" class="flex flex-col items-center">
                                            <div class="block bg-gray-50 hover:bg-gray-200 cursor-pointer w-[35px] h-[35px] rounded-full p-2 flex items-center justify-center">
