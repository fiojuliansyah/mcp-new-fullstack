        ...
        <div class="space-y-10">
            <div class="rounded-[21px] border border-[#2A2A2A] h-full">
                ...
                @if ($student->parent)
                    ...
                    <div class="grid grid-cols-7 gap-5 p-5">
                        <div class="col-span-7 lg:col-span-1 flex items-center justify-center">
                            @if ($student->parent->avatar_url)
                                <img src="{{ $student->parent->avatar_url }}" alt="Icon"
                                    class="rounded-full w-[109px] h-[109px] flex items-center justify-center">
                            @else
                                <div class="bg-gray-50 rounded-full w-[109px] h-[109px] flex items-center justify-center">
                                    <img src="/admin/assets/icons/student-black.svg" alt="Icon" class="w-10 text-black">
                        ...
                        <div class="col-span-7 lg:col-span-6">
                            <div class="grid grid-cols-2 gap-5">
                                <div class="col-span-2 flex flex-col">
                                    ...
                                    <span class="text-[#EAEAEA]">{{ $student->parent->name ?? '-' }}</span>
                                </div>
                                <div class="col-span-2 lg:col-span-1 flex flex-col">
                                    <span class="text-gray-200">Email</span>
                                    <span class="text-[#EAEAEA]">{{ $student->parent->email ?? '-' }}</span>
                                </div>
                                <div class="col-span-2 lg:col-span-1 flex flex-col">
                                    <span class="text-gray-200">Status Account</span>
                                    <span class="text-green-100">Active</span>
            ...
            <div class="space-y-5">
                <div class="flex items-center justify-between gap-3">
                    <div>
                        ...
                        <p class="text-gray-910">{{ $student->subscriptions->flatMap->classrooms->count() }} Subjects</p>
                    </div>
                </div>
                ...
                <div class="overflow-x-auto border border-[#424242] rounded-[12px] mb-10">
                    <table class="min-w-full border border-[#424242] rounded-[12px] overflow-hidden text-sm text-left">
                        ...
                        <tbody class="bg-gray-900 divide-y divide-gray-800 text-gray-50">
                                ...
                                @foreach ($subscription->classrooms as $classroom)
                                    @php
                                        ...
                                        $attendedCount = $schedules->sum(function ($schedule) use ($student) {
                                            return $schedule->attendances->isNotEmpty() ? 1 : 0;
                                        });
                                        $attendanceRate =
                                            $totalSchedules > 0 ? round(($attendedCount / $totalSchedules) * 100) : 0;
                                        ...
                                        $avgQuizScore =
                                            $totalQuizzesWithAttempts > 0
                                                ? round($totalQuizScore / $totalQuizzesWithAttempts)
                                                : 0;
                                        ...
                                        foreach ($schedules as $schedule) {
                                            foreach ($schedule->replays as $replay) {
                                                foreach ($replay->replayVideos as $video) {
                                                    $videoViews = $video->views->where('user_id', $student->id);
                                                    ...
                                                    if ($videoDuration > 0 && $videoViews->isNotEmpty()) {
                                                        $watchPercentage = min(
                                                            100,
                                                            round(($maxWatchedDuration / $videoDuration) * 100),
                                                        );
                                        ...
                                        $avgReplayWatchRate =
                                            $totalReplayCount > 0
                                                ? round($totalReplayPercentage / $totalReplayCount)
                                                : 0;
                                    ...
                                    <tr class="odd:bg-[#141414] even:bg-[#171717]">
                                        ...
                                        <td class="p-4 border border-gray-700 text-sm">
                                            Attendance: <span
                                                class="font-semibold text-green-400">{{ $attendanceRate }}%</span>
                                            <br>
                                            Replay: <span
                                                class="font-semibold text-blue-400">{{ $avgReplayWatchRate }}%</span>
                                        </td>
                                        <td class="p-4 border border-gray-700 text-sm">
                                            Avg Quiz Score: <span
                                                class="font-semibold text-yellow-400">{{ $avgQuizScore }}%</span>
                                            <br>
                                            <span class="text-gray-400">({{ $totalQuizAttempts }} attempts)</span>
                                        </td>
                                        <td class="p-4 border border-gray-700">
                                        ...
                                        <td>
                                            <div class="flex items-center justify-center gap-3">
                                                <a href="{{ route('admin.users.student.detail', $subscription->user->id) }}" target="_blank" class="flex flex-col items-center">
                                                    <div
                            ...
                            @if ($no === 1)
                                <tr>
                                    <td colspan="12" class="text-center py-6 text-gray-500">No subjects enrolled</td>
                                </tr>
                            @endif
                        </tbody>
                    </table>
