    ...
    <div class="space-y-5 py-5 px-10">
        <div class="space-y-7 flex flex-col">
            ...
            <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
                <div>
                    <h6 class="font-[600]">Tutor Lists</h6>
                    <p class="text-gray-910">{{ $count }} Tutors</p>
                </div>
                <a href="{{ route('admin.users.tutors.create') }}"
                    class="bg-gray-50 hover:bg-gray-200 rounded-full text-sm px-5 py-3 cursor-pointer">
                    <span class="text-black text-[16px] font-semibold">Add New Tutor</span>
                </a>
            </div>
        </div>
                    ...
                    <table class="min-w-full border border-[#424242] rounded-[12px] overflow-hidden text-sm text-left">
                        <thead class="bg-gray-800 text-gray-200">
                            <tr>
                                ...
                                <th class="p-4 border border-[#424242]">Total Subjects</th>
                                <th class="p-4 border border-[#424242]">Total Students</th>
                                <th class="p-4 border border-[#424242]">Avg Attendance</th>
                        ...
                        <tbody class="bg-gray-900 divide-y divide-gray-800 text-gray-50">
                            @forelse ($users as $index => $user)
                                <tr class="odd:bg-[#141414] even:bg-[#171717]">
                                    <td class="p-4 border border-gray-700">{{ $users->firstItem() + $index }}</td>
                                    <td class="p-4 border border-gray-700">{{ $user->name ?? '-' }}</td>
                                    <td class="p-4 border border-gray-700">{{ $user->email ?? '-' }}</td>

                                    <td class="p-4 border border-gray-700 text-center">
                                        <span class="font-semibold text-blue-300">{{ $user->total_subjects }}</span>
                                    </td>
                                    <td class="p-4 border border-gray-700 text-center">
                                        <span class="font-semibold text-yellow-300">{{ $user->total_students }}</span>
                                    </td>
                                    <td class="p-4 border border-gray-700 text-center">
                                        <span class="font-semibold
                                            {{ $user->avg_attendance >= 80 ? 'text-green-400' :
                                            ($user->avg_attendance >= 50 ? 'text-yellow-400' : 'text-red-400') }}">
                                            {{ $user->avg_attendance }}%
                                        </span>
                                    </td>
                                    ...
                                    <td class="p-4 border border-gray-700">
                                        <div class="flex items-center justify-center gap-3">
                                            <a href="{{ route('admin.users.tutors.show', $user->id) }}" class="flex flex-col items-center">
                                                <div
                                            ...
                                            <a href="{{ route('admin.users.tutors.edit', $user->id) }}" class="flex flex-col items-center">
                                                <div
                                                    class="block bg-gray-50 hover:bg-gray-200 cursor-pointer w-[35px] h-[35px] rounded-full p-2 flex items-center justify-center">
                            ...
                            @empty
                                <tr>
                                    <td colspan="8" class="text-center py-6 text-gray-500">No Tutors found.</td>
                                </tr>
                            @endforelse
                        </tbody>
