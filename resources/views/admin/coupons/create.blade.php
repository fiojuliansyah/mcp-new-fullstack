    ...
    <div class="space-y-7 flex flex-col">
        ...
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
            <div class="flex items-center gap-3">
                <a href="{{ route('admin.coupons.index') }}" class="bg-white rounded-full w-8 h-8 flex items-center justify-center cursor-pointer">
                    <img src="/admin/assets/icons/arrow-left.svg" alt="Icon" class="size-4">
                </a>
                <h6 class="text-gray-100 font-[600]">Create New Coupon</h6>
            </div>
        </div>
    </div>
        ...
        <form action="{{ route('admin.coupons.store') }}" method="POST" class="space-y-10">
            ...
            <div class="grid grid-cols-12 gap-10 pt-4">
                ...
                <div class="col-span-12 lg:col-span-6">
                    ...
                    <div class="relative">
                        <select name="type"
                            class="appearance-none pr-10 bg-gray-1000 border border-gray-950 text-gray-75 rounded-[14px] w-full px-4 py-3 @error('type') border-red-500 @enderror" required>
                            <option value="">Select Type</option>
                            <option value="percentage" @if(old('type') == 'percentage') selected @endif>Percentage (%)</option>
                            <option value="fixed" @if(old('type') == 'fixed') selected @endif>Fixed Amount (RM)</option>
                        </select>
                ...
                <div class="col-span-12 lg:col-span-6">
                    ...
                    <input type="number" name="max_uses_per_user" placeholder="Max times a single user can use it (Default: 1)"
                ...
                <div class="col-span-12 lg:col-span-6">
                    <label class="block mb-2 text-[15px] font-medium text-gray-200">Min Purchase Amount (Optional)</label>
                ...
                <div class="col-span-12">
                    ...
                    <textarea name="description" placeholder="Internal description about this coupon" rows="3"
                        class="appearance-none bg-gray-1000 border border-gray-950 text-gray-75 rounded-[14px] w-full px-4 py-3 @error('description') border-red-500 @enderror">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-2 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="col-span-12">
                    <button type="submit"
                        ...
                        <span class="text-black text-[16px] font-semibold">Create Coupon</span>
                    </button>
                </div>
            </div>
        </form>
